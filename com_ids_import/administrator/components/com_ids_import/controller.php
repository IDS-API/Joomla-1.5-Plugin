...
class Ids_importController extends JController {
    ...
    function eldisImport(){
        ...
        $server="eldis";
        $GUID=preg_replace('/\s+/', '', $data['api_key']);
                ...
                { 
                    ...
                    foreach ($content_elements as $list_item) {
                        ...
                        if(!$this->getContentUrl($website_url[0])){
                            ...
                            $newrecord->metadata = 'robots='."\n".'author='.implode(',',$new_author['list-item']).'';
    ...
    public function getContentUrl($website_url){
        $db =& JFactory::getDBO();
        $db->setQuery("SELECT `id` FROM `#__content` WHERE `metadesc` LIKE '%$website_url%'");
